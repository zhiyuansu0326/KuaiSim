...
#         print('slate_documents',slate_documents)
...
class Recsim_KREnvironment_WholeSession_GPU(BaseRLEnvironment):
    '''
    ...
    def __init__(self, args):
        ...
        '''
        super(Recsim_KREnvironment_WholeSession_GPU, self).__init__(args)
        ...
        # Recsim
        self.item_kale = torch.load('/content/gdrive/MyDrive/KRLBenchmark-main/code/recsim/item_kale').to(args.device)[:,:-1]
        self.user_kale = torch.load('/content/gdrive/MyDrive/KRLBenchmark-main/code/recsim/user_kale').to(args.device)[:,:,:-1]
    # Recsim
    # def Recsim_get_response_prob(self, user_kale, item_kale):
    #     # user_kale [B,2,feedback_num](mean,var)
    #     # item_kale [B,candidate_num,feedback_num]
    #     user_kale_mean = user_kale[:,0,:].unsqueeze(1).repeat(1,item_kale.shape[1],1)
    #     user_kale_var = user_kale[:,1,:].unsqueeze(1).repeat(1,item_kale.shape[1],1)
    #     engagement_loc = (item_kale * user_kale_var
    #                         + (1 - item_kale) * user_kale_mean)
    #     # engagement_loc *= self._user_state.satisfaction
    #     engagement_scale = (item_kale * user_kale_var
    #                         + (1 - item_kale)
    #                             * user_kale_mean)
    #     log_engagement = np.random.normal(loc=engagement_loc.cpu().numpy(),
    #                                         scale=engagement_scale.cpu().numpy())
    #     prob = torch.exp(torch.tensor(log_engagement,device=user_kale.device))
    #     prob = torch.nn.Sigmoid()(prob).float()
    #     return prob
    ...
    def Recsim_get_response_prob(self, user_kale, item_kale):
        ...
        # user_kale [B,2,feedback_num](mean,var)
        # item_kale [B,candidate_num,feedback_num]
        (B,candidate_num,feedback_num) = item_kale.shape
        user_choc = user_kale[:,:,-1] # is_hate
        item_choc = item_kale[:,:,-1]
        user_kale = user_kale[:,:,0] # is_click
        item_kale = item_kale[:,:,0]


        user_kale_mean = user_kale[:,0].unsqueeze(1).repeat(1,item_kale.shape[1])
        user_kale_var = user_kale[:,1].unsqueeze(1).repeat(1,item_kale.shape[1])
        user_choc_mean = user_choc[:,0].unsqueeze(1).repeat(1,item_kale.shape[1])
        user_choc_var = user_choc[:,1].unsqueeze(1).repeat(1,item_kale.shape[1])
        engagement_loc = (item_kale * user_choc_mean
                            + (1 - item_kale) * user_kale_mean)
        # engagement_loc *= self._user_state.satisfaction
        engagement_scale = (item_kale * user_choc_var
                            + (1 - item_kale)
                                * user_kale_var)
        log_engagement = np.random.normal(loc=engagement_loc.cpu().numpy(),
                                            scale=engagement_scale.cpu().numpy())
        prob = torch.exp(torch.tensor(log_engagement,device=user_kale.device))
        prob = torch.nn.Sigmoid()(prob).float()
        prob = prob.unsqueeze(2).repeat(1,1,feedback_num)
        return prob
    ...
    def get_response(self, step_dict):
        ...
        # Recsim
        # user_kale [B,2,feedback_num]  self.user_kale [user_num,2,feedback_num] 
        # item_kale [B,candidate,feedback_num] self.item_kale [item_num, feedback_num]
        user_kale = self.user_kale[batch['user_id']]
        item_kale = self.item_kale[batch['item_id'].reshape(1,-1)].reshape(batch['user_id'].shape[0],-1,7)
        # torch.save(out_dict['probs'],'out1')

        out_dict['probs'] = self.Recsim_get_response_prob(user_kale,item_kale)
        # torch.save(out_dict['probs'],'out2')
