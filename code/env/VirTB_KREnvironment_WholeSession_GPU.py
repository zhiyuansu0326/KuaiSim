...
# VirTB
from VirTB.MAIL.MailPolicy import MailPolicy

class VirTB_KREnvironment_WholeSession_GPU(BaseRLEnvironment):
    '''
    ...
    def __init__(self, args):
        ...
        '''
        super(VirTB_KREnvironment_WholeSession_GPU, self).__init__(args)
        ...
        # VirTB
        self.VirTB_immediate_response_model = MailPolicy()
        self.VirTB_immediate_response_model.to(args.device).load()
        # self.VirTB_user = torch.load('/content/gdrive/MyDrive/KRLBenchmark-main/code/VirTB/user_feature').to(args.device)
        # self.VirTB_item = torch.load('/content/gdrive/MyDrive/KRLBenchmark-main/code/VirTB/video_feature').to(args.device)
    ...
    def get_response(self, step_dict):
        ...
        # torch.save(out_dict,'outdict')
        # de
        # self.VirTB_user_state = self.VirTB_user[batch['user_id']]

        out_dict['probs'] = self.VirTB_immediate_response_model.get_response_prob(batch['user_id'],batch['item_id'])
        ########################################
