...
class AbstractEnv(gym.Env, ABC):
    ...
    def get_response(self, items):
        # items [B,candidates_num]
        # output: [B,candidates_num,feedback_num=7]
        prob = torch.zeros_like(items)
        for i in range(items.shape[0]):
            prob[i,np.int16(self.rng.choice(self.config.num_products))] = 1
        prob = prob.unsqueeze(2).repeat(1,1,7).float().to(items.device)
        return prob
